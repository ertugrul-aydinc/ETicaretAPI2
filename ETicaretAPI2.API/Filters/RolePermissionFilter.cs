...
namespace ETicaretAPI2.API.Filters
{
    public class RolePermissionFilter : IAsyncActionFilter
    {
        ...
        public async Task OnActionExecutionAsync(ActionExecutingContext context, ActionExecutionDelegate next)
        {
            ...
            if (!string.IsNullOrEmpty(name) && name != "ertugrulaydinc")
            {
                var descriptor = context.ActionDescriptor as ControllerActionDescriptor;
                var attribute = descriptor?.MethodInfo.GetCustomAttribute(typeof(AuthorizeDefinitonAttribute)) as AuthorizeDefinitonAttribute;
